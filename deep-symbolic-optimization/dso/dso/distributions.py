...
class Gamma(BaseAutoDiffDistribution):
    ...
    def __init__(self, k: float, theta: float):
        ...
        self.k = k
        assert theta > 0, f"theta must be greater than 0, but got theta={theta}"
        self.theta = theta
        super().__init__()
...
class T(BaseAutoDiffDistribution):
    ...
    def __init__(self, degrees_of_freedom: int, loc: float, scale: float):
        assert degrees_of_freedom > 0, f"degrees_of_freedom must be greater than 0, but got degrees_of_freedom={degrees_of_freedom}"
        self.degrees_of_freedom = degrees_of_freedom
        assert scale > 0, f"scale must be greater than 0, but got scale={scale}"
        self.loc = loc
        self.scale = scale
        super().__init__()
...
class Mixture(BaseAutoDiffDistribution):
    ...
    def _assert_same_num_weights_and_distributions(weights, distributions):
        assert len(weights) == len(distributions), f"len(weights)={len(weights)} != len(distributions)={len(distributions)}"

    @staticmethod
    def _assert_weights_sum_to_one(weights):
        assert np.sum(weights) == 1, f"np.sum(weights)={np.sum(weights)} != 1"
